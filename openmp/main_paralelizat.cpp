...
void compute_integral_image(float *image,float *ii,int N,int M) {
    ...
    ii[0]=image[0];
   	//#pragma omp parallel for  shared(ii,image,chunk) private(i,j) schedule(static,chunk)
    for (i=1;i<N;i++) {
        for (j=1;j<M;j++) {
...
int main() {
    ...
    int chunk;
        ...
        FILE* image_in=fopen("fete.pnm","rb");
        ...
        *width=N;
        ...
        int ij;
        chunk = CHUNKSIZE;
        #pragma omp parallel for  shared(integral_image,integral_image_square,chunk) private(ij,i,j,scale) schedule(static,chunk)
        for (ij=0;ij<=(N-SAMPLE_SIZE+1)*(M-SAMPLE_SIZE+1);ij++) {
       ...
       // int k;
        ...
        //#pragma omp parallel for  shared(face_windows,r,g,b,chunk) private(i,j) schedule(static,chunk)
        for (k=0;k<face_windows.size();k++) {
         ...
         FILE* image_out_seq=fopen("out","wb");
